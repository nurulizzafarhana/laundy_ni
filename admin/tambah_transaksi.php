...
$role = $_SESSION['role']; // jika role juga disimpan di session
...
while ($data_paket = mysqli_fetch_array($qry_paket)) {
    $paket_options .= '<option value="' . $data_paket['id_paket'] . '" data-harga="' . $data_paket['harga'] . '">' . $data_paket['nama_paket'] . '</option>';
}
							...
							<div class="menu menu-column menu-fit menu-rounded menu-title-gray-600 menu-icon-gray-400 menu-state-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-500 fw-bold fs-5 my-5 mt-lg-2 mb-lg-0" id="kt_aside_menu" data-kt-menu="true">
								<div id="kt_aside_menu_wrapper" class="menu-fit hover-scroll-y me-lg-n5 pe-lg-5" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-height="auto" data-kt-scroll-offset="20px" data-kt-scroll-wrappers="#kt_aside_wordspace">
									...
									<div class="menu-item">
										...
										<a class="menu-link" href="admin/detail_transaksi.php">
												<span class="menu-bullet">
													<span class="bullet bullet-dot"></span>
												</span>
					...
					<div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
						...
						<div class="container-fluid d-flex align-items-stretch justify-content-between" id="kt_header_container">
							...
							<div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
								<!--begin::Heading-->
								<h1 class="text-dark fw-bolder my-1 fs-2">Tambah Transaksi Pembayaran</h1>
								<!--end::Heading-->
								<!--begin::Breadcrumb-->
                                                        ...
                                                        <div class="container">
                                                            <div class="row">
                                                                    ...
                                                                    <div class="col-md-6 mb-2">
                                                                        ...
                                                                        <input id="kode_transaksi" name="kode_transaksi" class="form-control" type="text" value="<?php echo "TR-" . generateTransactionCode() ?>" readonly>
                                                                    </div>
                                                                    ...
                                                                    <div class="mb-2">
                                                                        ...
                                                                        <div class="mb-2">
																			...
																			<select name="id_user" class="form-control">
																				<option value="<?= $id_user ?>"><?= $nama_user ?> | <?= $role ?></option>
																				<!-- Tidak perlu menampilkan pilihan lain jika ini adalah pilihan otomatis -->
																			</select>
																		</div>

                                                                        <div class="mb-2">
																			<strong><label for="status">Status Transaksi</label></strong>
                                                                        ...
                                                                        <div class="mb-2">
                                                                            <button id="counterBtn" type="button" class="btn btn-primary btn-sm mb-2">Tambah Nama Paket</button>
                                                                                <table class="table">
                                                                                    <thead>
                                                                                        <tr>
                                                                                            <th>No</th>
																		...
																		<!-- <div class="form-group mb-2">
																			<label for="jumlah_dibayar">Nominal Bayar</label>
																			<input type="number" id="jumlah_dibayar" class="form-control" name="jumlah_dibayar" required>
																		</div>
																		<div class="form-group mb-2">
		...
		<!--begin::Global Javascript Bundle(used by all pages)-->
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
					...
					// Input jumlah dibayar dan kembalian
					...
					button.addEventListener('click', function() {
						...
						let newRow = "<tr>";
						newRow += "<td>" + no + "</td>";
						newRow += "<td><select class='form-control paket-select' name='id_paket[]' required>";
						newRow += "<option value=''>--Pilih Paket--</option>";
						newRow += `<?= $paket_options; ?>`; // Menggunakan PHP untuk opsi paket
						newRow += "</select></td>";

						// Kolom Harga (readonly) dan Qty (editable)
						newRow += "<td><input type='number' name='harga[]' class='form-control harga-input' readonly></td>";
						newRow += "<td><input type='number' name='qty[]' class='form-control qty-input' min='1' value='1'></td>";
						newRow += "<td><input type='number' name='sub_total[]' class='form-control sub-total' readonly></td>";
						newRow += "</tr>";

						tbody.insertAdjacentHTML('beforeend', newRow);

						// Ambil elemen select, harga, qty, dan subtotal pada baris baru
						const paketSelect = tbody.querySelector("tr:last-child .paket-select");
						const hargaInput = tbody.querySelector("tr:last-child .harga-input");
						const qtyInput = tbody.querySelector("tr:last-child .qty-input");
						const subTotalInput = tbody.querySelector("tr:last-child .sub-total");

						// Fungsi untuk menghitung subtotal berdasarkan harga dan qty
						function updateSubtotal() {
							const harga = parseFloat(hargaInput.value) || 0;
							const qty = parseInt(qtyInput.value) || 0;
							subTotalInput.value = harga * qty;
							updateTotal(); // Update total setelah subtotal dihitung
						}

						// Event listener saat paket dipilih, untuk mengisi kolom harga
						paketSelect.addEventListener('change', function() {
							const selectedOption = paketSelect.options[paketSelect.selectedIndex];
							const harga = selectedOption.getAttribute('data-harga');
							hargaInput.value = harga ? harga : ""; // Setel harga ke kolom harga
							updateSubtotal(); // Perbarui subtotal
						});

						// Event listener saat qty berubah
					...
					function updateTotal() {
						let total = 0;
						const subTotalInputs = document.querySelectorAll('.sub-total');
						subTotalInputs.forEach(function(input) {
							total += parseFloat(input.value) || 0;
						});

						// Menampilkan total pada field yang sesuai
						...
						if (jumlahDibayarInput.value) {
							const jumlahDibayar = parseFloat(jumlahDibayarInput.value);
							const kembalian = jumlahDibayar - total;
							kembalianInput.value = kembalian >= 0 ? kembalian : 0; // Pastikan kembalian tidak negatif
						}
					}

					// Event listener untuk jumlah dibayar
					jumlahDibayarInput.addEventListener('input', function() {
						updateTotal(); // Perbarui total dan kembalian saat jumlah dibayar diubah
					});
				});
            </script>
