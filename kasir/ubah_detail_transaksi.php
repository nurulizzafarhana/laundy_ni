	...
	<body id="kt_body" class="header-fixed header-tablet-and-mobile-fixed aside-fixed aside-secondary-enabled">
						...
						<div class="aside-workspace overflow-auto my-2 my-lg-7 px-3 px-lg-7" id="kt_aside_wordspace">
							...
							<div class="menu menu-column menu-fit menu-rounded menu-title-gray-600 menu-icon-gray-400 menu-state-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-500 fw-bold fs-5 my-5 mt-lg-2 mb-lg-0" id="kt_aside_menu" data-kt-menu="true">
								<div id="kt_aside_menu_wrapper" class="menu-fit hover-scroll-y me-lg-n5 pe-lg-5" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-height="auto" data-kt-scroll-offset="20px" data-kt-scroll-wrappers="#kt_aside_wordspace">
									...
									<div class="menu-item">
										<a class="menu-link active" href="./kasir/ubah_detail_transaksi.php">
											<span class="menu-bullet">
												<span class="bullet bullet-dot"></span>
											</span>
									...
									<div class="mb-18">
										...
										<div class="mb-10">
                                            ...
                                            <form action="./kasir/proses_ubah_detail_transaksi.php" method="post">
												<?php 
												include "koneksi.php";
												// Ambil data transaksi dan detail transaksi
												...
												$detail_query = "
													SELECT dt.id_detail_transaksi, p.nama_paket, dt.qty, p.harga
													FROM detail_transaksi dt
													JOIN paket p ON dt.id_paket = p.id_paket
													WHERE dt.id_transaksi = '" . $_GET['id_transaksi'] . "'";
												$detail_result = mysqli_query($conn, $detail_query);
												?>

												<!-- Hidden input untuk mengirimkan id_transaksi -->
														...
														<tr>
															<td>
																...
																<select class="form-control" name="id_paket[]">
																	<option value="<?php echo $detail['id_detail_transaksi']; ?>"><?php echo $detail['nama_paket']; ?></option>
																</select>
															</td>
															<td>
																<label for="" class="form-label">Harga</label>
																<input type="number" class="harga-input form-control" value="<?php echo $detail['harga']; ?>" readonly>
															</td>
															<td>
																<label for="" class="form-label">Qty</label>
																<input type="number" class="qty-input form-control" name="qty[]" value="<?php echo $detail['qty']; ?>" required>
															</td>
															<td>
																<label for="" class="form-label">Sub Total</label>
																<input type="number" class="sub-total form-control" value="<?php echo $detail['harga'] * $detail['qty']; ?>" readonly>
															</td>
														</tr>
													<?php endwhile; ?>
												</table>
												...
												<div class="mb-3">
													<strong><label class="col-lg-4 col-form-label" for="jumlah_dibayar">Nominal Bayar</label></strong>
													<input type="text" id="jumlah_dibayar" name="jumlah_dibayar" class="form-control" 
														placeholder="Masukkan jumlah bayar" 
														value="<?php echo ($data['status'] != 'baru') ? 'Rp ' . number_format($data['dibayar'], 0, ',', '.') : $data['dibayar']; ?>" 
														<?php echo ($data['status'] != 'baru') ? 'readonly' : ''; ?> />
												</div>

												<div class="mb-3">
													<strong><label class="col-lg-4 col-form-label" for="kembalian">Kembalian</label></strong>
													<input type="text" id="kembalian" name="kembalian" class="form-control" 
														placeholder="Kembalian" 
														value="<?php echo ($data['status'] != 'baru') ? 'Rp ' . number_format($data['kembalian'], 0, ',', '.') : $data['kembalian']; ?>" 
														<?php echo ($data['status'] != 'baru') ? 'readonly' : ''; ?> />
												</div>


												<div class="mb-3">
													<strong><label class="col-lg-4 col-form-label" for="tgl_bayar">Tanggal Bayar</label></strong>
													<input type="date" class="form-control" name="tgl_bayar" 
														value="<?php echo $data['tgl_bayar']; ?>" 
														<?php echo ($data['status'] != 'baru') ? 'readonly' : ''; ?> />
												</div>

												<div class="mb-3">
													<strong><label class="col-lg-4 col-form-label" for="status">Status Order</label></strong>
													<select class="form-control" name="status" <?php echo ($data['status'] == 'selesai') ? 'disabled' : ''; ?> required>
												...
												<div class="form-group row">
													<div class="col-lg-8 ml-auto">
														...
														<a href="./kasir/detail_transaksi.php?id_transaksi=<?=$data['id_transaksi']?>">
		...
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				...
				// Menghitung total harga dari qty dan harga
				function hitungTotalHarga() {
					...
					rows.forEach(row => {
						...
						const subTotalInput = row.querySelector('.sub-total');
						
						const harga = parseFloat(hargaInput.value) || 0;
						const qty = parseInt(qtyInput.value) || 0;
						
						// Menghitung subtotal dan menampilkannya
				...
				// Update kembalian saat nominal bayar berubah
				jumlahDibayarInput.addEventListener('input', function() {
					// Hitung total harga yang harus dibayar
					const totalHarga = hitungTotalHarga();

					// Ambil nominal bayar
					const nominalDibayar = parseFloat(jumlahDibayarInput.value) || 0;

					// Hitung kembalian
					const kembalian = nominalDibayar - totalHarga;

					// Perbarui nilai kembalian
					kembalianInput.value = kembalian >= 0 ? kembalian : 0;  // Jika kembalian negatif, set 0
				});

				// Update subtotal ketika qty berubah
				tbody.addEventListener('input', function(e) {
					if (e.target.classList.contains('qty-input')) {
						hitungTotalHarga(); // Perbarui total harga jika qty berubah
					}
				});
			});
